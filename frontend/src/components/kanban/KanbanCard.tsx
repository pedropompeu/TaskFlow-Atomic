...
export function KanbanCard({ card, onDelete, onEdit }: KanbanCardProps) {
  ...
  return (
    <div
      ...
      style={style}
      className={cn(
        'group bg-white rounded-lg border border-gray-200 p-3 shadow-sm cursor-grab active:cursor-grabbing',
    ...
    >
      ...
      <p
        className="text-sm font-medium text-gray-800 mb-2 leading-snug cursor-pointer hover:text-blue-600"
        onPointerDown={(e) => e.stopPropagation()}
        onClick={(e) => {
          e.stopPropagation();
          onEdit(card);
      ...
      <div className="flex items-center justify-between gap-2 mt-2">
        {card.dueDate && (
          ...
          >
            <Calendar size={11} />
            {formatDistanceToNow(parseISO(card.dueDate), { addSuffix: true })}
          </span>
        )}
        {card.assignedTo && (
