...
const ASSIGNEE_COLORS = [
  '#F78E2F', '#A559FD', '#43AC8D', '#1D84B7', '#FDCC32', '#999999',
];
...
function StatCard({ label, value, icon }: { label: string; value: number; icon: React.ReactNode }) {
  return (
    <div className="bg-white/70 backdrop-blur-sm rounded-xl border border-atomic-gray-300/20 p-4 flex items-center gap-4 shadow-sm hover:-translate-y-1 hover:shadow-[0_4px_20px_rgba(247,142,47,0.10)] transition-all duration-200">
      <div className="p-2.5 bg-atomic-orange/10 rounded-lg text-atomic-orange">{icon}</div>
      <div>
        <p className="text-2xl font-bold text-atomic-dark">{value}</p>
        <p className="text-xs text-atomic-gray-500 mt-0.5">{label}</p>
      </div>
    </div>
  );
}
...
function ChartShell({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="bg-white/70 backdrop-blur-sm rounded-xl border border-atomic-gray-300/20 p-5 shadow-sm">
      <h3 className="text-sm font-semibold text-atomic-dark/70 mb-4">{title}</h3>
      {children}
    </div>
  );
}
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-end gap-4 justify-between">
        <div>
          ...
          <p className="text-sm text-atomic-gray-500 mt-0.5">Visão geral de desempenho dos cards</p>
        </div>
        ...
        <div className="flex flex-wrap items-end gap-3">
          <div className="flex flex-col gap-1">
            ...
            <select
              ...
              className="px-3 py-2 text-sm border border-stone-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-atomic-orange bg-white"
            >
          ...
          <div className="flex flex-col gap-1">
            <label className="text-xs font-medium text-atomic-gray-500">De</label>
            <input
              type="date"
              value={startDate}
              onChange={(e) => setStartDate(e.target.value)}
              className="px-3 py-2 text-sm border border-stone-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-atomic-orange"
            />
          </div>

          <div className="flex flex-col gap-1">
            <label className="text-xs font-medium text-atomic-gray-500">Até</label>
            <input
              type="date"
              value={endDate}
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 animate-pulse">
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-20 bg-atomic-ice rounded-xl" />
          ))}
        </div>
      ) : (
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <ChartShell title="Cards por Status">
              {cardsByStatus.length === 0 ? (
                <p className="text-sm text-atomic-gray-500/70 text-center py-10">Sem dados no período</p>
              ) : (
                <ResponsiveContainer width="100%" height={240}>
                  <PieChart>
                    ...
                    <Legend
                      formatter={(value) => (
                        <span className="text-xs text-atomic-gray-600">{value}</span>
                      )}
                    />
                  </PieChart>
                </ResponsiveContainer>
          ...
          <ChartShell title="Conclusões ao longo do tempo">
            {(data?.completionsOverTime ?? []).length === 0 ? (
              <p className="text-sm text-atomic-gray-500/70 text-center py-10">
          ...
          {/* Overdue cards */}
          <div className="bg-white/70 backdrop-blur-sm rounded-xl border border-atomic-gray-300/20 p-5 shadow-sm">
            <h3 className="text-sm font-semibold text-atomic-dark/70 mb-4 flex items-center gap-2">
            ...
            {overdueCount === 0 ? (
              <p className="text-sm text-atomic-gray-500/70 text-center py-6">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="text-left text-xs text-atomic-gray-500/70 uppercase tracking-wider border-b border-stone-100">
                  ...
                  <tbody className="divide-y divide-atomic-ice">
                    {(data?.overdueCards ?? []).map((card) => {
                      ...
                      return (
                        <tr key={card.id} className="hover:bg-atomic-ice/60 transition-colors">
                          ...
                          <td className="py-2.5 pr-4 text-atomic-gray-600">
